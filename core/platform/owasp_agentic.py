...
_RISKS: list[RiskEntry] = [
    RiskEntry(
        ...
        mechanisms=[
            "labels: once a session carries content the agent did not author, only "
            "destinations declared before that content arrived are permitted. This is the "
            "deterministic part — a hijacked goal cannot choose where data goes",
            "injection_detector: keyword regex + semantic embedding scan on external content. "
            "Heuristic: it raises the cost of hijacking, it does not prevent it",
            "mcp_descriptor_guard: blocks poisoned MCP tool descriptions on first sight and "
            "on mutation, before they reach the model's context",
            "purpose_engine: alignment score drops when the inferred goal diverges from the "
            "declared purpose. Heuristic, and scored rather than enforced",
        ],
        notes=(
            "Detecting a hijacked goal means recognising adversarial text, which no "
            "scanner does reliably — prompt injection is an open problem. What AgentGate "
            "enforces deterministically is the consequence: once a session carries "
            "content the agent did not author, only destinations declared beforehand are "
            "permitted, so a hijacked goal cannot choose where data goes. The hijack may "
            "succeed; the exfiltration does not."
        ),
        # Not FULL. Detecting a hijacked goal means recognising adversarial text,
        # which no scanner does reliably — OpenAI calls prompt injection an open
        # problem. What is deterministic here is the consequence: the flow rule
        # stops hijacked intent reaching an undeclared destination. Claiming
        # deterministic pre-execution enforcement of the hijack itself would be
        # the first thing a technical reviewer disproves.
    ),
    RiskEntry(
        code="ASI02:2026",
