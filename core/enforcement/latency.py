...
Usage:
    from core.enforcement import latency
...
def measure(component: str):
    ...
    """
    # perf_counter, not monotonic: monotonic's resolution is about 15.6 ms on
    # Windows, so anything faster than a frame records as zero and the
    # percentiles become noise on the platform this is developed on.
    t0 = time.perf_counter()
    try:
        yield
    finally:
        record(component, (time.perf_counter() - t0) * 1000.0)
