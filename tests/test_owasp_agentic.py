...
import time
import unittest
from core.platform.owasp_agentic import (
    CoverageLevel,
    RiskEntry,
    get_risks,
    get_risk,
...
class TestSpecificRiskCoverage(unittest.TestCase):
    ...
    def test_asi01_goal_hijack_is_partial_not_full(self):
        """
        Goal hijack is PARTIAL on purpose, and this test exists to keep it there.

        FULL is defined in this module as deterministic pre-execution enforcement.
        Detecting a hijacked goal means recognising adversarial text, which no
        scanner does reliably. The flow rule does enforce the consequence
        deterministically — a hijacked goal cannot pick an undeclared destination
        — and that is what PARTIAL records. Upgrading this to FULL without a
        mechanism that stops the hijack itself is the claim a reviewer disproves
        first.
        """
        risk = self._get("ASI01:2026")
        assert risk.coverage == CoverageLevel.PARTIAL
        assert "open problem" in risk.notes
    ...
    def test_six_full_four_partial_zero_none(self):
        """Pinned so a grade cannot drift upward without someone deciding to."""
...
class TestComplianceReport(unittest.TestCase):
    ...
    def test_generated_at_is_recent_utc(self):
        ...
        # timegm, not mktime: the string is UTC and mktime reads it as local time,
        # so this test only passed on a machine set to UTC and was off by exactly
        # the timezone offset everywhere else.
        ts_epoch = calendar.timegm(ts)
    ...
    def test_summary_counts_match_expected(self):
        ...
        assert s["risks_fully_covered"] == 6
        assert s["risks_partially_covered"] == 4
        assert s["risks_not_covered"] == 0
        assert s["total_risks"] == 10

    def test_coverage_score_is_85_percent(self):
        s = generate_compliance_report()["summary"]
        assert s["coverage_score_pct"] == 80.0
    ...
    def test_report_is_serialisable_as_json(self):
        ...
        assert roundtrip["summary"]["coverage_score_pct"] == 80.0
