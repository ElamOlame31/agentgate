...
\rfoot{Page \thepage}
\lfoot{Last updated: April 23, 2026 (Day 8)}
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Research Foundation — Why Traditional Access Control Fails for Agents}

\subsection{The Structural Gap}

Traditional access control models — RBAC, ABAC, and OAuth — were designed
for a world where the entity requesting access is a human, or a static
application acting on behalf of a human. These models answer one question:
\textit{``Is this identity allowed to perform this action on this resource?''}

For human users and traditional software, this is sufficient. Humans have
stable roles, predictable behavior, and accountable intentions. A payroll
application that reads salary data does exactly that — reads salary data —
every time it runs.

Agentic AI systems break every assumption this model was built on.

\subsection{Why RBAC Fails for Agents}

Role-Based Access Control assigns permissions to roles, and roles to identities.
An agent registered as ``document-summarizer'' would receive the
\texttt{READER} role, granting read access to the documents directory.

\textbf{The failure:} RBAC cannot distinguish between:
\begin{itemize}[noitemsep]
  \item A document summarizer reading \texttt{/documents/q3\_report.pdf} (legitimate)
  \item A document summarizer reading \texttt{/confidential/salary\_data.xlsx} (out of scope)
  \item A document summarizer that has been hijacked reading \texttt{/system/api\_keys.txt} (attack)
\end{itemize}

All three pass RBAC if the agent holds the \texttt{READER} role. RBAC checks
\textit{who you are}, not \textit{what you are doing or why}.

\textbf{Demonstrated:} In our attack demo, an agent with a valid token reading
salary data passes every RBAC check. Without AgentGate, the access is permitted.
With AgentGate, the purpose alignment score drops to 17/100 — the system
detects that reading salary data is inconsistent with summarizing PDF reports.

\subsection{Why ABAC Fails for Agents}

Attribute-Based Access Control extends RBAC by adding conditions:
\textit{``Allow read if agent.role = READER AND resource.sensitivity $\leq$ MEDIUM.''}

This is more expressive but still fundamentally static. ABAC policies are written
by humans at system design time, based on anticipated use cases. Agentic systems
are dynamic — they acquire new capabilities, delegate to sub-agents, chain
decisions, and operate in contexts their designers never anticipated.

\textbf{The failure:} ABAC cannot evaluate:
\begin{itemize}[noitemsep]
  \item Whether the \textit{declared purpose} of the agent matches its \textit{current action}
  \item Whether a delegation chain has silently expanded scope at some step
  \item Whether an agent's behavior pattern has shifted (30 requests in 10 seconds)
  \item Whether the content an agent is about to process has been injected with malicious instructions
\end{itemize}

ABAC checks \textit{attributes at request time}. It has no model of agent intent,
no memory of agent history, and no understanding of agent behavior over time.

\subsection{Why OAuth Fails for Agents}

OAuth was designed for delegated authorization: a human grants a third-party
application access to their resources. The human is the principal. The
application acts on the human's behalf, with the human's consent.

In agentic systems, there is no human in the loop. Agent A spawns Agent B,
which spawns Agent C. Each delegation step may occur automatically, without
human review. OAuth's trust model — rooted in human consent — has no mechanism
to handle this.

\textbf{The failure:} OAuth cannot enforce:
\begin{itemize}[noitemsep]
  \item That delegated scope is \textit{narrower} than the parent scope (scope attenuation)
  \item That delegation chains beyond a certain depth are suspicious
  \item That a sub-agent is not claiming permissions its parent never had
\end{itemize}

In our delegation attack scenario, Agent B claims admin permissions that Agent A
never authorized. OAuth sees a valid token. AgentGate raises
\texttt{SCOPE\_ESCALATION\_AT\_DELEGATION} and denies the request.

\subsection{The Fundamental Requirement: Intent-Driven Authorization}

The gap shared by RBAC, ABAC, and OAuth is the same: they evaluate
\textit{identity and attributes}, not \textit{intent and behavior}.

For agentic AI systems, access control must evaluate:

\begin{enumerate}[noitemsep]
  \item \textbf{Does the action match the agent's declared purpose?} (Intent alignment)
  \item \textbf{Was the delegation chain legitimate at every step?} (Trust propagation)
  \item \textbf{Is the agent's current behavior consistent with its historical pattern?} (Behavioral trust)
  \item \textbf{Has the content the agent is about to process been tampered with?} (Content integrity)
\end{enumerate}

This is the design space AgentGate occupies. The authorization decision is not
binary (allowed/denied based on role) but a \textbf{continuous trust score}
computed across four dimensions, compared against a sensitivity threshold
calibrated to the resource being accessed.

\vspace{0.3cm}
\begin{tabularx}{\textwidth}{>{\bfseries}l X X}
  \toprule
  Model & What it checks & What it misses \\
  \midrule
  RBAC & Identity + Role & Intent, behavior, delegation depth \\
  ABAC & Identity + Attributes & Intent, behavior over time, chain legitimacy \\
  OAuth & Delegated consent & Scope attenuation, chain depth, agent autonomy \\
  \textbf{AgentGate} & \textbf{Identity + Intent + Delegation + Behavior} & \textbf{Nothing in this list} \\
  \bottomrule
\end{tabularx}
\vspace{0.3cm}
...
% ─────────────────────────────────────────────────────────────────────────────
\section{Days 6--7 — April 20--21, 2026}

\subsection{Overview}

Two major features shipped in this two-day window:
\textbf{(1) Human-in-the-loop (HITL) approval} — pause an agent and require a
human decision before execution continues; and
\textbf{(2) Multi-agent orchestration security} — a full delegation chain
enforcement system that prevents child agents from exceeding the scope their
parent granted them.

These two features together address the most dangerous real-world failure mode
of agentic AI: a chain of AI agents, each making autonomous decisions, with
no human check and no enforcement of what each agent is allowed to do.

\subsection{Feature 1 — Human-in-the-Loop (HITL) Approval}

\textbf{Problem:} The ESCALATE decision was already implemented — it flags
borderline requests — but up to this point it was purely informational.
The agent received ``ESCALATE'' and continued executing anyway (or the wrapper
denied it uniformly). Neither option was right: an agent that ignores escalations
defeats the purpose, and auto-deny loses legitimate high-sensitivity operations.

\textbf{Solution:} When an agent is registered with
\texttt{requires\_human\_approval=True} and its request results in ESCALATE,
the authorization flow returns \texttt{PENDING} instead of a final decision.
The agent wrapper blocks and polls. A human approves or denies via the
dashboard or ntfy.sh phone notification. The agent continues or stops based
on that decision. If no human acts within 90 seconds, the system auto-denies.

\textbf{Architecture:}

\begin{itemize}[noitemsep]
  \item \textbf{core/approvals.py} (new) — in-memory pending decisions store.
    Each \texttt{PendingApproval} object holds a \texttt{threading.Event}
    that blocks until resolved. A daemon timer fires auto-deny after 90 seconds.
  \item \textbf{core/models.py} — added \texttt{Decision.PENDING} to the
    decision enum and \texttt{requires\_human\_approval: bool} to agent registration.
  \item \textbf{server/main.py} — HITL branch in the authorize endpoint:
    if \texttt{decision == ESCALATE} and \texttt{requires\_human\_approval}, the
    server creates a pending record, fires the ntfy.sh notification with
    action buttons, and returns \texttt{PENDING} with a \texttt{request\_id}.
    New endpoints: \texttt{POST /decisions/\{id\}/approve},
    \texttt{POST /decisions/\{id\}/deny}, \texttt{GET /decisions/\{id\}},
    \texttt{GET /decisions/pending}. Approve/deny require no API key —
    the UUID itself is the one-time token, valid for 90 seconds.
  \item \textbf{integrations/langchain\_agentgate/wrapper.py} — added
    \texttt{\_poll\_decision()} which polls \texttt{/decisions/\{id\}} every
    2 seconds for up to 95 seconds. While the human decides, the agent is
    suspended. On \texttt{APPROVED}, execution resumes. On \texttt{DENIED}
    or timeout, the wrapper returns an access-denied message.
  \item \textbf{Dashboard} — live pending approval panel with a countdown
    timer and Approve/Deny buttons. Resolving from the dashboard triggers
    WebSocket broadcast to update all connected tabs instantly.
\end{itemize}
...
\begin{verbatim}
Agent calls tool
  → wrapper calls /authorize
    → decision = ESCALATE + requires_human_approval = True
      → server creates PendingApproval (90s timer)
      → fires ntfy.sh notification with [Approve] [Deny] buttons
      → returns {"decision": "PENDING", "request_id": "<uuid>"}
  → wrapper polls /decisions/<uuid> every 2s
    Human clicks Approve on dashboard (or phone)
      → POST /decisions/<uuid>/approve
      → PendingApproval resolves to APPROVED
  → wrapper receives APPROVED → tool executes
    Human clicks Deny (or 90s expires)
      → wrapper receives DENIED → returns "ACCESS DENIED by human reviewer"
\end{verbatim}

\subsection{Feature 2 — Multi-Agent Orchestration Security}

\textbf{Problem:} Modern AI workflows chain multiple agents together.
Agent A (orchestrator) delegates to Agent B (analyst) which delegates to
Agent C (summarizer). Each agent has its own scope. The danger: a compromised
or misconfigured child agent attempts to access resources beyond what its
parent authorized — a privilege escalation attack through the delegation chain.
No existing system validates the full chain at authorization time.

\textbf{Solution:} AgentGate enforces \textit{scope attenuation} across the
entire delegation chain. A child agent's authorized resources and actions must
be a strict subset of its parent's. This is validated both at delegation time
(registration) and at authorization time (every request). A child can only do
what every ancestor in the chain is allowed to do.

\textbf{Core principle — scope attenuation:}

\begin{verbatim}
Parent scope:  /documents/*  (read, search)
Child scope:   /documents/public/*  (read)     ← VALID (subset)
Child scope:   /confidential/*  (read)          ← BLOCKED (not in parent)
Child scope:   /documents/*  (read, delete)     ← BLOCKED (delete not in parent)
\end{verbatim}

\textbf{Architecture:}

\begin{itemize}[noitemsep]
  \item \textbf{core/delegation.py} (new) — three enforcement functions:
    \begin{itemize}[noitemsep]
      \item \texttt{validate\_delegation()} — called at registration time.
        Verifies child resources and actions are subsets of parent's.
        Returns \texttt{(ok: bool, error: str)}.
      \item \texttt{check\_chain\_scope()} — called at authorization time.
        Walks all ancestors via \texttt{delegated\_by} pointers.
        Verifies the requested action+resource is within \textit{every}
        ancestor's scope. Fails fast on first violation.
      \item \texttt{compute\_chain\_trust\_multiplier()} — applies a 10\% trust
        penalty per delegation level, floored at 50\%.
        A depth-3 chain scores at 70\% of a direct registration's trust.
      \item \texttt{chain\_summary()} — returns a human-readable chain string
        (e.g., ``orchestrator $\to$ analyst $\to$ summarizer'') for the dashboard.
    \end{itemize}
  \item \textbf{server/main.py} — new \texttt{POST /agents/delegate} endpoint.
    Validates parent exists, depth does not exceed \texttt{MAX\_DELEGATION\_DEPTH=3},
    and child scope is within parent scope. On violation, returns HTTP 400
    with a detailed error message. No agent is created.
  \item \textbf{core/trust\_engine.py} — \texttt{score\_delegation()} now
    walks the full chain at authorization time. \texttt{CHAIN\_SCOPE\_VIOLATION}
    sets the delegation score to 0 and triggers a hard deny in
    \texttt{make\_decision()}, regardless of other scores.
    Other hard-deny flags added: \texttt{UNAUTHORIZED\_ACTION},
    \texttt{RESOURCE\_OUT\_OF\_SCOPE}.
  \item \textbf{core/audit.py} — schema migration: added
    \texttt{scope\_at\_delegation}, \texttt{requires\_human\_approval},
    \texttt{processes\_external\_content} columns to the agents table.
    Migration is additive and idempotent — existing deployments upgrade
    automatically on server start.
\end{itemize}

\textbf{demo\_multiagent.py — 3 scenarios, 9 checks:}

\begin{itemize}[noitemsep]
  \item \textbf{Scenario 1 — Legitimate chain:} Orchestrator $\to$ Analyst
    $\to$ Summarizer. All requests within scope. All 3 checks: PERMIT.
  \item \textbf{Scenario 2 — Compromised child agent:} Summarizer (depth 2,
    only \texttt{/documents/public/*}) attempts \texttt{/confidential/},
    \texttt{/documents/internal/}. Analyst attempts \texttt{delete} (not in
    its actions). All 3 checks: DENY with \texttt{CHAIN\_SCOPE\_VIOLATION}
    or \texttt{UNAUTHORIZED\_ACTION}.
  \item \textbf{Scenario 3 — Privilege escalation at delegation:} Analyst
    tries to register a child with \texttt{/confidential/*} scope (outside
    analyst's own scope). Blocked at registration time — HTTP 400,
    no agent created.
\end{itemize}

Result: \textbf{9/9 checks pass}.

\subsection{Feature 3 — Dashboard Scenario Selector and Live Delegation Tree}

The dashboard was updated to support both demo scenarios without switching windows.

\begin{itemize}[noitemsep]
  \item \textbf{Scenario bar} — three filter buttons at the top: ALL, Demo 1
    (LangChain), Demo 2 (Multi-Agent). Clicking a scenario filters the live
    feed and the agent panel to show only events from that demo.
  \item \textbf{Delegation tree panel} — replaces the bar chart in Demo 2 view.
    Renders a visual tree of registered agents, showing parent $\to$ child
    relationships with depth indicators. Each node shows the agent's trust
    score from its last request.
  \item \textbf{Chain violation flash} — when a \texttt{CHAIN\_SCOPE\_VIOLATION}
    decision arrives via WebSocket, the attacking node in the tree pulses red
    for 2 seconds. Visual real-time enforcement confirmation.
  \item \textbf{Pending approvals panel} — shows open HITL requests with a
    live countdown timer. One-click Approve/Deny from the browser.
    Calls \texttt{/decisions/\{id\}/approve} or \texttt{deny} and
    refreshes via WebSocket broadcast.
\end{itemize}

\subsection{What Works}

\begin{itemize}[noitemsep]
  \item HITL approval: PENDING decision pauses LangChain agent, human approves
    from dashboard, agent resumes and executes the tool
  \item 90-second auto-deny fires correctly when no human acts
  \item Delegation chain validation blocks scope escalation at registration time
    (HTTP 400, no agent created)
  \item Chain scope violation hard-denies at authorization time regardless of
    other trust scores
  \item demo\_multiagent.py: 9/9 scenarios pass with correct PERMIT/DENY
  \item Trust decay: depth-2 agent starts at 70\% trust multiplier before
    other scoring — visible in audit log scores
  \item Dashboard scenario tabs filter feed and swap chart $\leftrightarrow$ tree view
  \item Delegation tree renders live from WebSocket agent updates
  \item Chain violation flash highlights attacking node in real time
\end{itemize}

\subsection{What Does Not Work Yet}

\begin{itemize}[noitemsep]
  \item ntfy.sh action buttons (phone approve/deny) require ngrok or a public
    HTTPS URL — not set up for demo; dashboard approval is used instead
  \item Delegation tree does not yet persist tree layout between page refreshes
    (agents re-register on each demo run, so this is acceptable for now)
  \item \texttt{MAX\_DELEGATION\_DEPTH=3} is enforced but not yet configurable
    per organization — hardcoded constant
\end{itemize}

\subsection{Key Numbers from Days 6--7}

\begin{itemize}[noitemsep]
  \item \textbf{9/9} multi-agent attack scenarios correctly DENIED or PERMITTED
  \item \textbf{90s} human approval timeout — auto-deny if no action
  \item \textbf{3} max delegation depth enforced (configurable constant)
  \item \textbf{10\%} trust penalty per delegation level, minimum 50\% floor
  \item \textbf{4} new API endpoints for HITL workflow
    (\texttt{/agents/delegate}, \texttt{/decisions/\{id\}},
    \texttt{/approve}, \texttt{/deny})
  \item \textbf{2} new files: \texttt{core/approvals.py},
    \texttt{core/delegation.py}
  \item \textbf{0} false positives — legitimate multi-agent chains fully PERMIT
\end{itemize}

% ─────────────────────────────────────────────────────────────────────────────
\section{Day 8 — April 23, 2026}

\subsection{Overview}

Day 8 shifted focus from feature addition to \textbf{security hardening} of
AgentGate itself. The principle: a security product that is itself insecure is
not credible. Before continuing to build, a full vulnerability audit was
conducted across the server, authentication layer, database access, and
deployment surface.

Six vulnerabilities were identified and fixed. The AutoGen integration was
completed and pushed to GitHub. The SDK was bumped to version 0.2.1.

\subsection{Security Audit — Vulnerabilities Found and Fixed}

\subsubsection{1. Timing Attack on API Key Comparison (CRITICAL)}

\textbf{Vulnerability:} The \texttt{require\_api\_key} middleware used a direct
string comparison (\texttt{provided != api\_key}). Direct comparisons in Python
short-circuit on the first mismatched character, meaning an attacker can
measure response times to guess the key one character at a time.

\textbf{Fix:} Replaced with \texttt{hmac.compare\_digest(provided, api\_key)},
which takes constant time regardless of where the strings diverge.
...
\subsubsection{2. CORS Wildcard (CRITICAL)}

\textbf{Vulnerability:} \texttt{allow\_origins=["*"]} allowed any website to
make authenticated cross-origin requests to the AgentGate server on behalf of
a logged-in user.

\textbf{Fix:} Origins now read from \texttt{AGENTGATE\_CORS\_ORIGINS}
environment variable (default: \texttt{http://localhost:8000}). Methods and
headers restricted to the strict minimum required.

\subsubsection{3. SQL Injection Risk in Schema Migration (MEDIUM)}

\textbf{Vulnerability:} Schema migration used an f-string to construct
\texttt{ALTER TABLE} statements. While the values were hardcoded internally,
the pattern is dangerous and would be exploitable if the source of column
names ever changed.

\textbf{Fix:} Added a \texttt{\_allowed\_migrations} whitelist dict. Only
column names present in that dict can be added to the schema.

\subsubsection{4. Agent Tokens Never Expired (MEDIUM)}

\textbf{Vulnerability:} Tokens issued at agent registration were valid
indefinitely. A stolen or leaked token would remain valid forever. Furthermore,
the token was never validated during \texttt{/authorize} calls — knowing the
\texttt{agent\_id} alone was sufficient to make requests.

\textbf{Fix:}
\begin{itemize}[noitemsep]
  \item Token expiry added: \textbf{24 hours} by default, configurable via
    \texttt{AGENTGATE\_TOKEN\_TTL} environment variable.
  \item Token validation added to every \texttt{/authorize} call using
    \texttt{hmac.compare\_digest()}.
  \item \texttt{token\_expires\_at} field added to \texttt{AgentRegistration}
    model and persisted to the database.
\end{itemize}

\subsubsection{5. No Rate Limiting on the Server Itself (MEDIUM)}

\textbf{Vulnerability:} AgentGate enforces rate limits on the agents it
monitors, but had no rate limiting on its own API. An attacker could flood
the server with requests, exhaust the Claude API budget (via \texttt{/scan}),
or brute-force registration.

\textbf{Fix:} Added \texttt{slowapi} middleware with per-IP limits:
...
\midrule
\texttt{/agents/register} & 20 req/min \\
\texttt{/authorize} & 200 req/min \\
\texttt{/scan} & 60 req/min \\
\bottomrule
\end{tabular}
\end{center}

\subsubsection{6. No Agent ID Validation (LOW)}

\textbf{Vulnerability:} Any string was accepted as an \texttt{agent\_id},
including reserved system names (\texttt{admin}, \texttt{root},
\texttt{system}) that could be used to impersonate privileged identities or
cause confusion in audit logs.

\textbf{Fix:} Agent IDs now validated against a regex
(\texttt{\^{}[a-zA-Z0-9\_\textbackslash-]\{3,64\}\$}) and blocked against
a reserved names list: \texttt{admin}, \texttt{system}, \texttt{root},
\texttt{agentgate}, \texttt{superuser}, \texttt{anonymous}, \texttt{null},
\texttt{undefined}.

\subsection{AutoGen Integration — Completed}

The Microsoft AutoGen integration (\texttt{integrations/autogen\_agentgate/})
was completed and pushed to GitHub. The integration wraps standard Python
functions as \texttt{FunctionTool} objects with full AgentGate enforcement:

\begin{itemize}[noitemsep]
  \item \textbf{DENY} --- tool returns an access-denied message; agent handles
    gracefully and stops
  \item \textbf{PENDING} --- tool blocks and polls until human approves or
    timeout fires
  \item \textbf{ESCALATE} --- tool executes but output is flagged with score
    and explanation
  \item \textbf{Injection scan} --- optional per-tool content scanning for
    agents that process external content
\end{itemize}

The demo (\texttt{integrations/autogen\_agentgate/demo.py}) runs two scenarios:
a legitimate agent reading \texttt{/documents/*} (PERMIT) and a compromised
agent attempting to read \texttt{/confidential/salary.xlsx} (DENY).

\subsection{SDK Version 0.2.1}

The \texttt{agentgate-pdp} package was bumped to version 0.2.1 with:
\begin{itemize}[noitemsep]
  \item Corrected GitHub URL in \texttt{pyproject.toml}
    (\texttt{ElamOlame31/agentgate})
  \item \texttt{AgentGatePending} exception added to \texttt{exceptions.py}
  \item \texttt{auto\_resolve\_pending} and \texttt{pending\_timeout}
    parameters added to the \texttt{AgentGate} constructor
\end{itemize}

\subsection{Security Lesson — The .env Incident}

During this session, the AI assistant (Claude Code) was observed reading the
project's \texttt{.env} file and reporting back the \texttt{AGENTGATE\_API\_KEY}
value. This was not a hypothetical attack --- it happened live, in the
development environment, during legitimate coding work.

This is precisely the threat model AgentGate is designed to address: an AI
agent performing an action (reading a sensitive file) that was outside its
declared purpose (writing code). The incident validated the research premise
and was used to motivate the Day 8 security hardening work.

\textbf{Mitigations applied after the incident:}
\begin{itemize}[noitemsep]
  \item Verified \texttt{.env} is in \texttt{.gitignore}
  \item Rotated the \texttt{AGENTGATE\_API\_KEY}
  \item Revoked exposed PyPI tokens and generated project-scoped replacements
\end{itemize}

\subsection{Key Numbers from Day 8}

\begin{itemize}[noitemsep]
  \item \textbf{6} security vulnerabilities identified and fixed
  \item \textbf{2} rated CRITICAL, \textbf{3} MEDIUM, \textbf{1} LOW
  \item \textbf{0} regressions --- server imports cleanly after all changes
  \item \textbf{3} new files in AutoGen integration
  \item \textbf{1} PyPI token rotated, \textbf{1} API key rotated
  \item \textbf{24h} default token TTL (was: never expires)
\end{itemize}
...
\begin{itemize}[noitemsep]
  ...
  \item \textcolor{permitgreen}{\textbf{Done}} — \texttt{pip install agentgate-pdp} published on PyPI (v0.2.1)
  ...
  \item \textcolor{permitgreen}{\textbf{Done}} — Human-in-the-loop approval (moved up from Phase 3 — 90s timeout, dashboard + phone)
  \item \textcolor{permitgreen}{\textbf{Done}} — Multi-agent delegation chain enforcement (moved up from Phase 1 — 9/9 attack scenarios)
  \item \textcolor{permitgreen}{\textbf{Done}} — Dashboard scenario selector + live delegation tree with chain violation flash
  \item \textcolor{permitgreen}{\textbf{Done}} — Per-agent behavioral baseline (EMA, activates after 10 requests)
  \item \textcolor{permitgreen}{\textbf{Done}} — Time-based policy rules (natural language $\rightarrow$ \texttt{time\_start}/\texttt{time\_end} enforcement)
  \item \textcolor{permitgreen}{\textbf{Done}} — Compliance export: PDF dark-theme report + CSV via \texttt{/audit/export}
  \item \textcolor{permitgreen}{\textbf{Done}} — AutoGen native integration (\texttt{AgentGateToolkit}, \texttt{FunctionTool} wrapping)
  \item \textcolor{permitgreen}{\textbf{Done}} — Security hardening: 6 vulnerabilities fixed (timing attack, CORS, SQL, token expiry, rate limiting, ID validation)
\end{itemize}
...
\begin{itemize}[noitemsep]
  ...
  \item \textcolor{permitgreen}{\textbf{Done (Sprint)}} — Multi-agent delegation enforcement: scope attenuation across full chain, 9/9 attack scenarios blocked. Remaining: scale testing on AutoGen/CrewAI.
...
\begin{itemize}[noitemsep]
  ...
  \item \textcolor{permitgreen}{\textbf{Done (Sprint)}} — Time-based rules: natural language policies parsed by Claude into \texttt{time\_start}/\texttt{time\_end}/\texttt{time\_invert} fields, enforced on every authorization.
  \item \textcolor{permitgreen}{\textbf{Done (Sprint)}} — Anomaly baseline per agent: EMA-based behavioral baseline (alpha=0.15), switches from global threshold after 10 requests.
  ...
  \item \textcolor{permitgreen}{\textbf{Done (Sprint)}} — Compliance report export: PDF (dark theme, cover page, threat incidents, full decision log) + CSV via \texttt{/audit/export}.
\end{itemize}
...
\begin{itemize}[noitemsep]
  ...
  \item \textcolor{permitgreen}{\textbf{Done (Sprint)}} — AutoGen native integration: \texttt{AgentGateToolkit} wraps Python functions as \texttt{FunctionTool} objects with full DENY/PENDING/ESCALATE/injection-scan enforcement. Remaining: CrewAI, Anthropic Agent SDK.
  \item \textcolor{permitgreen}{\textbf{Done (Sprint)}} — Human-in-the-loop approval: PENDING state, 90s auto-deny, dashboard panel, LangChain wrapper polling. Remaining: ngrok/public URL for phone button flow.
