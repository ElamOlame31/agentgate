...
% ─────────────────────────────────────────────────────────────────────────────
\section{Day 3 — (Planned)}

\begin{itemize}[noitemsep]
  \item Connect a real Claude agent (using Anthropic SDK) to AgentGate — agent calls \texttt{/authorize} before every tool use
  \item Live demo: real LLM deciding to summarize documents, AgentGate evaluating each file access in real time
  \item Add \texttt{ESCALATE} notification system (simulate an alert email/webhook)
\end{itemize}
